...
__global__ void softmax_kernel(const float* x, float* y) {
    __shared__ float scratch[128];
    int lid = threadIdx.x;
    scratch[lid] = x[lid];
    __syncthreads();
    for (int stride = 64; stride > 0; stride >>= 1) {
        if (lid < stride) scratch[lid] = fmaxf(scratch[lid], scratch[lid + stride]);
        __syncthreads();
    }
    float e = expf(x[lid] - scratch[0]);
    scratch[lid] = e;
    __syncthreads();
    for (int stride = 64; stride > 0; stride >>= 1) {
...
int main(int argc, char** argv) {
    ...
    snprintf(in_path, sizeof(in_path), "%s/softmax_input.raw.f32", argv[1]);
    ...
    softmax_kernel<<<1, 128>>>(d_x, d_y);
    cudaError_t status = cudaDeviceSynchronize();
    cudaMemcpy(h_y, d_y, n * sizeof(float), cudaMemcpyDeviceToHost);
    if (write_f32(out_path, h_y, n)) return 4;
    FILE* jf = fopen(json_path, "w");
    fprintf(jf, "{\"kernel\":\"softmax\",\"status\":\"%s\",\"cuda_status\":\"%s\",\"elements\":%d,\"output_fnv1a64\":\"%016llx\"}\n",
