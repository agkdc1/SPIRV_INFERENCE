...
def make_tensor(builder, name, shape, buffer_index):
    ...
    builder.PrependInt8Slot(1, 0, 0)  # TensorType_FLOAT32
    builder.PrependUint32Slot(2, buffer_index, 0)
    builder.PrependUOffsetTRelativeSlot(3, name_off, 0)
    return builder.EndObject()


def make_operator_code(builder):
    builder.StartObject(4)
    builder.PrependInt8Slot(0, 14, 0)  # BuiltinOperator_LOGISTIC in old schemas
    builder.PrependInt32Slot(3, 14, 0)  # builtin_code in newer schemas
    builder.PrependInt32Slot(2, 1, 0)
    return builder.EndObject()


def make_operator(builder):
    inputs = i32_vector(builder, [0])
    outputs = i32_vector(builder, [1])
...
def build_model():
    ...
    desc = builder.CreateString("synthetic float32 logistic model for cuda-spirv loader smoke test")
    input_tensor = make_tensor(builder, "input", [1, 8], 0)
    output_tensor = make_tensor(builder, "output", [1, 8], 1)
    tensors = table_vector(builder, [input_tensor, output_tensor])
    inputs = i32_vector(builder, [0])
    outputs = i32_vector(builder, [1])
    op = make_operator(builder)
    operators = table_vector(builder, [op])
    ...
    opcodes = table_vector(builder, [make_operator_code(builder)])
    subgraphs = table_vector(builder, [subgraph])
    buffers = table_vector(builder, [make_buffer(builder), make_buffer(builder)])
